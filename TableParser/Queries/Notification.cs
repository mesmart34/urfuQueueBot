        ...
        public void StartPolling()
        {
            ...
            var th = new Thread(() =>
            {
                ...
                while (_started)
                {
                    ...
                    var firstRoomDef = RoomParser.GetRoomCellIdByContent(data, "Защита", 0);
                    var secondRoomDef = RoomParser.GetRoomCellIdByContent(data, "Защита", 1);

                    int skipRows = 0;
                    int offset = 0;
                    for (; offset < data[0].Count; ++offset)
                    {
                        var roomInfoRow = RoomParser.GetRoomCellIdByContent(data, "Защита", offset);
                                ...
                                case NotificationType.AUTO:
                                    {
                                        ...
                                        //DateTime currentTeamStartTime = _room.StartTime + team.Id * avgTimeSeconds;
                                        var deltaTime = (currentTime - prevTeamStartTime).TotalSeconds;
